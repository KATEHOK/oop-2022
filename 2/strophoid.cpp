...
namespace strophoid {
	...
	*/
	...
	Strophoid::Strophoid(const long double A) { setA(A); }

	void Strophoid::setA(const long double A) { this->A = A; }

	long double Strophoid::getA() const {
		return this->A;
	}

	long double Strophoid::getY(const long double X) const {
		return abs(X) * sqrt( abs( (this->A + X) / (this->A - X) ) );
	}

	long double Strophoid::getRadius(long double angle) const {
	...
	long double Strophoid::getRadiusOfCurvature() const {
		return this->A * sqrt(2);
	}

	long double Strophoid::getLoopArea() const {
		return pow(this->A, 2) * (2 - 0.5 * M_PI);
	}

	long double Strophoid::getLoopVolume() const {
		return M_PI * pow(this->A, 3) * (2 * log(2) - 4 / 3);
	}

	long double Strophoid::getMiddleBranchesArea() const {
		return pow(this->A, 2) * ( 2 + 0.5 * M_PI);
	}
}